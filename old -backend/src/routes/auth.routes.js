...
const { supabase, supabaseUser } = require('../config/supabase');
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (branch_id) {
      ...
      if (branch.status !== 1) {
        return res.status(400).json({ error: 'Selected branch is currently inactive' });
      }
    }
...
router.post('/logout', requireAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to log out from server' });
  }
});
